        ...
        if(empty($_POST['nomClient']) and empty($_POST['prenom']) and empty($_POST['cni']) and empty($_POST['villed']) and empty($_POST['dated']) and empty($_POST['heured']) and empty($_POST['tel']) and empty($_POST['mail']))
         {
            ...
            $sqlInsert = 'INSERT INTO achatligne (DateDepart, HeureDepart, TypeBillet, fkIdClient, fkIdVilleD) VALUES ("'.$date.'","'.$heure.'","CLASSIQUE","'.$Rech7.'","'.$ville.'")'; 
...
<body>
  <?php  
echo "<button type=\"button\" class=\"btn btn-outline-secondary\"><a href=\"achat-classique-form.php\">Retour</a></button>";
?>
<br><br><br>
  <!-- Contenu de la page-->
<div class="container">
